...
export default function FileUpload() {
  ...
  function handleFileInputChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (files !== null) {
      ...
      const newFiles = [...selectedFiles, ...newSelectedFiles] as uploadedFile[]
  ...
  return (
    <motion.div className={styles.fileUploadContainer} layout>
      <motion.label htmlFor="fileInput" className={styles.customFileInput} layout>
        Choose file
      </motion.label>
      <input
        type="file"
      ...
      <motion.div className={styles.fileListContainer} layout>
        ...
        <AnimatePresence>
          {selectedFiles.map((file, index) => {
            return (
              <ListItem file={file} index={index} key={file.id} handleRemoveFile={handleRemoveFile}/>
            );
          })}
        </AnimatePresence>
      </motion.div>
